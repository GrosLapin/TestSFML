...
#include "outils/fonction_conteneur.hpp"
#include "route.hpp"
namespace testSFML 
{
	...
	bool is_angle_ok(const route<Point>& une_route, Point2 && point, double angles)
	{
					        ...
					        double distance_max = 1000)
        {
            ...
            static_assert(is_point<decltype (get_value<Conteneur1>(centres.begin()) )>::value, "Le centres ne contient pas de point");
            static_assert(is_point<decltype (get_value<Conteneur2>(points.begin()) )>::value, "Le points ne contient pas de point");
			...
			// j'ai pas envie de faire end() -1 car je connais pas mon type d'iterateur
			{
				...
				for (auto it = centres.begin() ; i < centres.size()-1 ; i++ , it = std::next(it) )
				{
					...
					const auto& centre = get_value<Conteneur1>(it);
					
					for ( auto itj  = std::next(centres.begin(),(long int)(i+1)); itj != centres.end() ; itj = std::next(itj) )
					{
						const auto& other = get_value<Conteneur2>(itj);
